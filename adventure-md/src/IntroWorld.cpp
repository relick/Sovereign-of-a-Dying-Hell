...
{
	Assert(m_script, "Must provide a script to IntroWorld");
}
...
)
{
	...
	else if (m_timer > FIX16(3.5))
	{
		...
		io_game.RequestNextWorld(std::make_unique<TitleWorld>(std::move(m_script)));
	}
	m_timer += FrameStep();
	...
	{
		m_lineTable[i] = F16_mul(sinFix16((i << 5) + m_sineScroll), m_timer - FIX16(1.75)) >> 4;
		if(i&1)
		{
