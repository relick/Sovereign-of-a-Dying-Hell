...
)
{
	if (m_context)
	{
		if (m_context->m_pose->m_animation[m_context->m_currentFrameIndex].m_frameDuration == 0)
		{
		...
		++m_context->m_ticks;
		if (m_context->m_ticks >= m_context->m_pose->m_animation[m_context->m_currentFrameIndex].m_frameDuration)
		{
